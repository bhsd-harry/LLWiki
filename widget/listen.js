...
(() => {
	const main = async () => {
		...
		let audio, vup, vdown; // eslint-disable-line prefer-const
		...
		const browser = $.client.profile().name;
		const menu = mw.menu([...browser === 'iphone'
			? []
			: [ // iOS不可通过JavaScript调节音量
			...
			if (menu.$element.width() > 45 + (skin ? 14 : 16) * (browser === 'iphone' ? 2 : 4)) {
				return;
			}
		...
		const $download = menu.findItemFromData(3).$label.wrap('<a>').parent();
