        ...
        if (name == 'wiki') {
            promise[ name ] = mw.config.get( 'extCodeMirrorConfig' ) ? Promise.resolve() : Promise.all([
                ...
                $.get({ dataType: 'script', cache: true,
                    url: '//cdn.jsdelivr.net/gh/bhsd-harry/LLWiki@latest/otherwiki/mediawiki.min.js'
                }).then(() => {
                    mw.loader.load('//cdn.jsdelivr.net/gh/bhsd-harry/LLWiki@latest/otherwiki/mediawiki.min.css', 'text/css');
                })
            ]);
        } else {
