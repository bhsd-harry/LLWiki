...
.runmode.linenums > ol { margin: 0 !important; list-style-position: outside; }
.runmode.linenums > ol > li { background-color: #fff; border-left: 1px solid #ddd; padding: 0 4px; margin: 0; }`,
		...
		codemirror_path = ['//fastly.jsdelivr.net/npm/codemirror@5.35.0/lib/codemirror.min.js',
			...
			'//fastly.jsdelivr.net/gh/bhsd-harry/LLWiki@2.20/otherwiki/gadget-site-lib.codemirror.min.js',
		],
		main = async $content => {
			if (contentModel) {
				$content.find('.mw-code').addClass(`hljs linenums ${contentModel}`);
			} else {
				$content.find('pre[lang], code[lang]').addClass(function() {
					const lang = langs[this.lang.toLowerCase()];
					if (lang) {
						return `hljs ${lang}${this.tagName === 'PRE' ? ' linenums' : ''}`;
					}
				});
			...
			if ($code.length) { // 高亮Wikitext
				...
				try {
					...
					mw.notify('无法下载CodeMirror扩展，Wikitext高亮失败！', {type: 'error'});
				}
			}
