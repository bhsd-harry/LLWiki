...
(async () => {
    ...
    if ($("body").hasClass("mw-special-Specialpages")) {
        $("#mw-specialpagesgroup-pagetools")
            .next(".mw-specialpages-list")
            .children("ul")
            .append(
                $("<li>")
                    .addClass("mw-specialpagerestricted")
                    .append(
                        $("<a>")
                            .text("批量回退编辑")
                            .attr("href", "/zh/Special:BatchRollback")
                            .attr("title", "Special:BatchRollback")
                    )
            );
        return;
    }

    // Only load on Special:BatchRollback
    if (mw.config.get("wgPageName") !== "Special:BatchRollback") {
        if (
            mw.config.get("wgPageName").toLowerCase() === "special:batchrollback" ||
            mw.config.get("wgPageName") === "Special:批量回退"
        ) {
            // He's a little confused, but he's got the spirit.
            window.location.replace("/zh/Special:BatchRollback");
        }
        return;
    }

    // Fake a special page
    $("body").removeClass("mw-invalidspecialpage").addClass("mw-special-BatchRollback");
    $("#firstHeading").text("批量回退");
    $("#mw-content-text").text("正在加载……");
    await mw.loader.using(["mediawiki.api", "mediawiki.jqueryMsg"]);
    const api = new mw.Api();
    await api.loadMessagesIfMissing(["pagetitle"]);
    document.title = mw.msg("pagetitle", "批量回退");
    ...
    const fieldset = new OO.ui.FieldsetLayout({
        ...
        classes: ["container"],
        help:
            "本工具用于批量回退指定用户编辑，可用于快速处理破坏。仅在用户具有rollback权限时有效。",
        helpInline: true,
    });
    ...
    // Emergency stop (basically reloads the page)
    $(emergency.$element).click(() => {
                        ...
                        await Promise.allSettled(
                            ids.map((cur, i) => {
                                ...
                                return api
                                    ...
                                    .catch((e) =>
                                        log(progress + "删除修订版本 " + cur + " 时发生错误：" + e)
                                    );
                            })
                        );
                    }
