...
if ((ns >= 0 && ns % 2 == 1 || pagename == 'Help:互助客棧') && ['view', 'submit'].includes(action)) {
	...
	const settings = mw.gadgets.CommentsInLocalTime,
		{lang} = settings, // 注意这是一个数组！提供一套默认的英文设置
	...
	mw.messages.set($.extend(wgULS({ // 这些消息用于小工具设置
		...
		? { 'gadget-lc-today': 'Today', 'gadget-lc-yesterday': 'Yesterday',
	...
	mw.settingsDialog.addTab({name: 'CommentsInLocalTime', label: 'gadget-lc-label', items: [
		...
		{key: 'utcoffset', type: 'Number', label: 'gadget-lc-offset', help: mw.msg('gadget-lc-offsetHelp'), config:
            {max: 14, min: -12, step: 1, inputFilter(num) {
            	return num.replace('+', '');
            }},
			...
			options: locales.map(function(ele) {
				return {data: ele};
			}), disabled: isEn,
			inputFilter(locale) {
				return locales.includes(locale) ? locale : moment.locale();
			}},
	...
	mw.hook('settings.dialog').add(function(params) {
		if (params.name != 'CommentsInLocalTime') {
			return;
		}
		console.log('Hook: settings.dialog, 开始调整小工具设置 - CommentsInLocalTime');
		const {items} = params,
			i = items.findIndex(function(ele) {
				return ele.key == 'lang';
			});
		items[i].widget.on('change', function(value) { // 使用英语的设置组合
			const disabled = value.length;
			items.slice(i + 1).forEach(function(ele) {
				ele.widget.setDisabled(disabled);
			});
			params.fields[0].items.forEach(function(ele) {
				ele.widget.setDisabled(disabled);
			});
		});
	});
