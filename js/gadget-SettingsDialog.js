...
const isUser = mw.config.get('wgUserGroups').includes('user'),
	...
	clearWidgets = function(arr, settings) { // 还原一组OOUI widget
		(arr || []).forEach(function(ele) {
			ele.widget.setValue(settings ? settings[ele.key] || '' : '');
		});
	},
	...
	buildForm = function(params, $element) {
		if (!params.ready) { // 生成表单，只需要执行一次，不用写成SettingsDialog的内置方法
			...
			$element.append((params.items || []).map(function(ele) {
				return buildWidget(ele, settings).$element;
			}));
			$element.append((params.fields || []).map(function(ele) {
				const field = new OO.ui.FieldsetLayout({ label: mw.msg(ele.label), help: ele.help, helpInline: true,
					items: (ele.items || []).map(function(e) {
						return buildWidget(e, settings[ele.key]);
					}) });
	...
	openDialog = function(e) {
		...
		dialog.open().opening.then(function() {
			buildForm(dialog.getObject(), dialog.getPanel().$element);
		});
	};
// 4. 定义SettingsDialog类
...
SettingsDialog.prototype.getActionProcess = function(action) {
	const dialog = this, // ES5不允许箭头函数，无法直接使用this关键字
		gadget = this.gadgets.filter(function(ele) {
			return ele.ready;
		}); // 忽略未加载的小工具
	if (action == 'save') {
		gadget.forEach(function(ele) {
			dialog.saveOptions(ele);
		});
	} else {
		gadget.forEach(function(ele) {
			dialog.clearOptions(ele);
		});
	}
	this.close();
...
SettingsDialog.prototype.addTab = function(params) {
	...
	panel.on('active', function() {
		buildForm(params, panel.$element);
	});
	if (ready) {
		return;
	}
...
SettingsDialog.prototype.getObject = function(arg) {
	...
	return this.gadgets.find(function(ele) {
...
SettingsDialog.prototype.clearOptions = function(arg) {
	...
	(gadget.fields || []).forEach(function(ele) {
		clearWidgets(ele.items, settings[ele.key]);
	});
};
/**
